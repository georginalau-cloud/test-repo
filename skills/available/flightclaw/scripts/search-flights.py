...
def format_results(results, currency, is_round_trip=False):
    ...
    for i, result in enumerate(results, 1):
        if is_round_trip and isinstance(result, tuple):
            # Result structure: ((outbound_flights_list, return_flights_list), booking_token)
            # or: ((FlightResult, FlightResult), booking_token) from raw parsing
            flight_data, booking_token = result
            flights_tuple = flight_data
            
            # Handle both formats: tuple of lists or tuple of individual flights
            if isinstance(flights_tuple[0], list):
                outbound_flights = flights_tuple[0]
                return_flights = flights_tuple[1]
                outbound = outbound_flights[0] if outbound_flights else None
                ret = return_flights[0] if return_flights else None
            else:
                outbound, ret = flights_tuple[0], flights_tuple[1]
            
            if outbound is None or ret is None:
                print(f"\n{'='*60}")
                print(f"Option {i}: data incomplete, skipping")
                continue
                
            total_price = outbound.price + ret.price
            print(f"\n{'='*60}")
            print(f"Option {i}: {fmt_price(total_price, currency)} total")
